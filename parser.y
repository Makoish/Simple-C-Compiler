...
stmnt:   assignment ';'
        | expr ';'
        | declaration 
        | var_type EQUAL VARIABLE { currFuncName = $3.name; TableOfFunctions->CheckFunctionReturn($1.type, $3.name, yylineno, &print); } '(' call_list { quadruples->print_call_function(fileName, TableOfFunctions, $3.name, indentation, $1.name); } ')' ';' {CallOrder = 0;}
        ...
        | VARIABLE EQUAL VARIABLE {currFuncName = $3.name; }  '(' call_list { quadruples->print_call_function(fileName, TableOfFunctions, $3.name, indentation, $1.name); }')' ';' {CallOrder = 0;}
        ...
        | func_type { quadruples->print_label(fileName, indentation, $1.name); indentation+=5;  } '(' { currFunc = new FunctionTable(); currFunc = currFunc->newFuncTable($1.name, curr); curr = currFunc; TableOfFunctions->addFunction(currFunc, $1.type); } parm_list ')' '{' stmnt_list ret {
                                                                                          if (strcmp($9.type, "NONE")){
                                                                                            
                                                                                                if (strcmp($1.type, $9.type) && print){
                                                                                                    print = false;
          ...
          '}' { indentation-=5;  quadruples->print_return(fileName, indentation, $9.value); }
        | SWITCH '(' VARIABLE       { quadruples->printSwitchLabel(fileName, indentation, $3.name); quadruples->addSwitchExit(); curr->checkCaseType($3.name, yylineno, &print); curr->checkTable($3.name, yylineno, &print); curr->checkInit($3.name, &print, yylineno);} 
...
rval:   INTEGER{$$.type = "int"; $$.value = $1.value; $$.name = $1.value; $$.REGISTER = $1.value;}
      ...
      | VARIABLE{
        ...
        curr->checkInit($1.name, &print, yylineno);
        $$.name = $1.name;
        $$.type = type;
        $$.value = $1.name;
        $$.REGISTER = $1.name;
...
expr: expr PLUS expr {curr->checkOperation($1.type, $3.type, yylineno, &print, "Adding"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printOperation(fileName, $1.REGISTER, $3.REGISTER, indentation, "+").c_str());}
      |expr MINUS expr {curr->checkOperation($1.type, $3.type, yylineno, &print, "Minusing"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printOperation(fileName, $1.REGISTER, $3.REGISTER, indentation, "-").c_str()); }
      |expr MULT expr {curr->checkOperation($1.type, $3.type, yylineno, &print, "Multplying"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printOperation(fileName, $1.REGISTER, $3.REGISTER, indentation, "*").c_str()); }
      |expr DIV expr {curr->checkOperation($1.type, $3.type, yylineno, &print, "Dividing"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printOperation(fileName, $1.REGISTER, $3.REGISTER, indentation, "/").c_str()); }    
      |expr POW expr {curr->checkOperation($1.type, $3.type, yylineno, &print, "Powering"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printOperation(fileName, $1.REGISTER, $3.REGISTER, indentation, "^").c_str()); }
      |expr AND expr {curr->checkOperation($1.type, $3.type, yylineno, &print, "Anding"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printOperation(fileName, $1.REGISTER, $3.REGISTER, indentation, "&&").c_str()); }      
      |expr OR expr  {curr->checkOperation($1.type, $3.type, yylineno, &print, "Oring"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printOperation(fileName, $1.REGISTER, $3.REGISTER, indentation, "||").c_str()); }    
      |expr EQUEQU expr {curr->checkOperation($1.type, $3.type, yylineno, &print, "EQUATING"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printOperation(fileName, $1.REGISTER, $3.REGISTER, indentation, "==").c_str()); } 
      |expr NOTEQU expr {curr->checkOperation($1.type, $3.type, yylineno, &print, "NOT EQUATING"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printOperation(fileName, $1.REGISTER, $3.REGISTER, indentation, "!=").c_str()); }
      |expr PLUSPLUS {curr->checkSingleOperation ($1.type, yylineno, &print, "PLUS PLUS"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printSingleOperation(fileName, $1.REGISTER, indentation, "++").c_str()); }
      |expr MINUSMINUS {curr->checkSingleOperation ($1.type, yylineno, &print, "MINUS MINUS"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printSingleOperation(fileName, $1.REGISTER, indentation, "--").c_str());} 
      |expr GT expr {curr->checkOperation($1.type, $3.type, yylineno, &print, "GREATER_THAN"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printOperation(fileName, $1.REGISTER, $3.REGISTER, indentation, ">").c_str()); }     
      |expr GTE expr {curr->checkOperation($1.type, $3.type, yylineno, &print, "GREATER_THAN_EQUAL"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printOperation(fileName, $1.REGISTER, $3.REGISTER, indentation, ">=").c_str()); } 
      |expr LT expr {curr->checkOperation($1.type, $3.type, yylineno, &print, "LESS_THAN"); $$.type = $1.type; $$.REGISTER = strdup(quadruples->printOperation(fileName, $1.REGISTER, $3.REGISTER, indentation, "<").c_str()); }    
      |expr LTE expr {curr->checkOperation($1.type, $3.type, yylineno, &print, "LESS_THAN_EQUAL"); $$.type = $1.type; }   
...
quadruples->print_var_type(fileName, indentation, $1.type, $2.name);
...
func_type: type VARIABLE {$$.type = $1.type; $$.name = $2.name;}| VOID VARIABLE {$$.type = "NONE"; $$.name = $2.name;};
...
cond:   cond EQUEQU cond { curr->checkOperation($1.type, $3.type, yylineno, &print, "EQUEQU"); $$.REGISTER = strdup(quadruples->print_Cond_Operation(fileName, $1.REGISTER, $3.REGISTER, indentation, "EQEQ").c_str()); }
      ...
      |VARIABLE {curr->checkTable($1.name, yylineno, &print); curr->checkInit($1.name, &print, yylineno); $$.type = strdup(curr->getTypebyName($1.name).c_str()); $$.REGISTER = $1.name;}
      |VARIABLE {currFuncName = $1.name; } '(' call_list ')' {CallOrder = 0; $$.REGISTER = $1.name;}
      |INTEGER { $$.type = "int"; $$.REGISTER = $1.value; };
       

post_loop:  expr PLUSPLUS {quadruples->printPostLoop(fileName, $1.REGISTER, "++", indentation);}
          | expr MINUSMINUS {quadruples->printPostLoop(fileName, $1.REGISTER, "--", indentation);};
